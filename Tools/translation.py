...
def parallel_deeptranslate(content: Series, src_lang: str, dst_lang: str, show_progress=True, max_workers: int = 4,
                           max_attempts=3):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in iterator:
            ...
            try:
                ...
                result[index] = llm_translate(content[index], src_lang)
    return result
...
def llm_translate(text: str, lang: str, model='qwen2.5:7b', verbose=False):
    ...
    try:
        ...
        if verbose:
            print(f'{h_yellow}Sentence: {text}{h_stop}')
...
def chunk_by_sentences(text, lang: str, max_length=5000):
    ...
    for sentence in sentences:
        if len(sentence) > max_length:
            word_chunks = chunk_by_words(sentence, max_length)
...
if __name__ == '__main__':
    # Translation
    text = (
        'In the heart of a bustling digital metropolis, where artificial intelligence and human creativity intertwine like vines in a neon forest, there existed a language no one could speak—but everyone tried to understand. It was not bound by syntax or semantics, nor was it written in books or databases. This language lived in code, danced in circuits, and echoed through the algorithms of minds both metal and mortal.',
    ...
    print(parallel_deeptranslate(
        ['البروفيسورة سارة جيلبرت قائدة فريق العمل في لقاح جامعة أكسفورد قامت بتطعيم ابنائها التوأم الثلاثة بلقاح #كورونا وهم يبلغون من العمر 21 سنة وجميعهم يدرسون الكيمياء الحيوية بنفس الجامعة ! جيلبرت : أنا لست قلقة ونحن نعمل إختبارات امنه للجميع https://t.co/blP6PGl9zX'],
        'ar', 'en'))
    print('Done')